...
//deben analizar las ciudades que tengan al menos 5 mediciones válidas

val ciudadValida = ciudades.filter(_.temps.size > 4)


def valorVariacion(temperaturas: List[Double]): Double = {
  val maxima: Double = temperaturas.max
  val minima: Double = temperaturas.min
  maxima - minima
}
//diferencia entre la temperatura máxima y la mínima de su lista temps
val variacion = ciudadValida.map(c =>valorVariacion(c.temps))
variacion
