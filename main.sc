...
def ciudadConMayorVariacion(ciudades: List[Ciudad]): String = {
  // Filtrar ciudades con más de 5 temperaturas
  val ciudadesFiltradas = ciudades.filter(_.temps.size >= 5)

  // Si no hay ciudades válidas, retornar un mensaje
  if (ciudadesFiltradas.isEmpty) {
    return "No hay resultados."
  }

  // Inicializar variables para la ciudad con mayor variación térmica
  var ciudadConMayorVariacion = ""
  var mayorDiferencia = Double.MinValue

  // Recorrer las ciudades filtradas
  for (ciudad <- ciudadesFiltradas) {
    val tempMaxima = ciudad.temps.max
    val tempMinima = ciudad.temps.min
    val diferencia = tempMaxima - tempMinima

    // Si la variación es mayor que la anterior, actualizamos
    if (diferencia > mayorDiferencia) {
      mayorDiferencia = diferencia
      ciudadConMayorVariacion = ciudad.nombre
    }
  }

  // Devolver el resultado
  s"La ciudad con la mayor variación térmica es: $ciudadConMayorVariacion con una diferencia de $mayorDiferencia grados."
}

// Llamada a la función
println(ciudadConMayorVariacion(ciudades))
